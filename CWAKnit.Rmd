...
# testing(CAD_Weight)
# 
# data_obs_t <- filament[filament$Class=="obs",]
# 
# formulas_t <- list(E = ~ 1 + CAD_Weight, V = ~ 1) #not sure about the formulas
# 
# 
# Z_t=model_Z(formulas_t,data_obs_t)
#   opt <- optim(rep(0, ncol(Z_2[["ZE"]])),
#                fn = neg_log_lik,
#                Z = Z_t, 
#                y = data_obs_t$Actual_Weight, #hacky
#                method = "BFGS", 
#                hessian = TRUE)
#   
#   theta_t <- opt$par
#   Sigma_theta_t <- solve(opt$hessian)
...
ggplot() +
  ...
  #geom_point(data = data_obs, aes(CAD_Weight, Actual_Weight)) +
